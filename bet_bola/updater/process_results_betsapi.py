...
def get_games(games):
    ...
    request = requests.get('https://api.betsapi.com/v1/bet365/result?token=20445-cCVWmEfcE9UQpu&event_id=' + games_ids_string)
    if request.status_code == 200:
        data_games = request.json()
        if data_games['success'] == 1:
            for index, game_json in enumerate(data_games['results']):
                if int(game_json.get('time_status', 0)) not in [0,1]:
...
def update_game_score(game_json, game_id):
    ...
    game.status = int(game_json.get('time_status', 2))
...
def process_games(games=None):
    if not games:
        ...
        .filter(~Q(score_half='', score_full='')).filter(status=3)
    ...
    for game in games:
        ...
        half_time_correct_score_NEW(game_scores, game.cotations.filter(market__name='Placar Correto 1° Tempo'))
        ...
        correct_score_NEW(game_scores, game.cotations.filter(market__name='Placar Correto'))
...
def half_time_correct_score(scores, cotations):
    if scores.get('1', None):
        ...
        if cotations.count() > 0:
            cotations.update(settlement=1)
            if home > away:
                cotations.filter(name__startswith='Casa').filter(name__contains=home_correct_score_string).update(settlement=2)
            elif home < away:
                cotations.filter(name__startswith='Fora').filter(name__contains=away_correct_score_string).update(settlement=2)
            else:
                cotations.filter(name__startswith='Empate').filter(name__contains=home_correct_score_string).update(settlement=2)


def half_time_correct_score_low(scores, cotations):
    if scores.get('1', None):
        home = int(scores['1']['home'])
        away = int(scores['1']['away'])
...
def half_time_correct_score_NEW(scores, cotations):
    if scores.get('1', None):
        home = int(scores['1']['home'])
        away = int(scores['1']['away'])
        ...
        if cotations.count() > 0:
            cotations.update(settlement=1)
            cotations.filter(name__startswith=correct_score_string).update(settlement=2)


def half_time_correct_score_NUMBER(scores, cotations):
    if scores.get('1', None):
        home = int(scores['1']['home'])
        away = int(scores['1']['away'])
        ...
        home_correct_score_string = '1 ' + home_correct_score_string
        away_correct_score_string = '2 ' + away_correct_score_string
        x_correct_score_string = 'X ' + home_correct_score_string

        if cotations.count() > 0:
            #cotations.update(settlement=1)
            if home > away:
                cotations.filter(name__startswith=home_correct_score_string).update(settlement=2)
            elif home < away:
                cotations.filter(name__startswith=away_correct_score_string).update(settlement=2)
            else:
                cotations.filter(name__startswith=x_correct_score_string).update(settlement=2)
...
def correct_score_NEW(scores, cotations):
    if scores.get('2', None):
        home = int(scores['2']['home'])
        away = int(scores['2']['away'])
...
def winning_margin(scores, cotations):
    if scores.get('2', None):
        ...
        if cotations.count() > 0:
            ...
            if latest_total_goals:
                if abs(goals_difference) >= latest_total_goals:
                    if goals_difference > 0:
                        cotations.filter(name__startswith='Casa', name__contains='+').update(settlement=2)
                    elif goals_difference < 0:
                        cotations.filter(name__startswith='Fora', name__contains='+').update(settlement=2)
