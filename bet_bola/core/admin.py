    ...
    def queryset(self, request, queryset):
        if self.value() == 'yes':
            ...
            games = Game.objects.filter(pk__in=games_ids)\
                .annotate( end_date = ExpressionWrapper(F('start_date') + timedelta(minutes=300), output_field=DateTimeField()) )\
...
class GameAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        if not obj.results_calculated:
            super().save_model(request, obj, form, change)
            if obj.score_half and obj.score_full:
                process_games([obj])
