...
class TicketView(FiltersMixin, ModelViewSet):
    ...
    def toggle_remove_cotation(self, request, pk=None):
        ...
        if cotation_copy.active:
            message = 'Cota adicionada com sucesso :)'
        else:
            message = 'Cota removida com sucesso :)'

        if CotationCopy.objects.filter(active=True, ticket__pk=ticket.pk).count() <=1 and not cotation_copy.active:
            return Response({
            'success': False,
