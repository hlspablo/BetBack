...
from core.serializers.market import (
    MarketCotationSerializer, 
    MarketSerializer,
    MarketCotationSerializerWithNoCotations
)
from core.serializers.cotation import CotationsByMarketSerializer
...
class MarketCotationView(CacheKeyDispatchMixin, ModelViewSet):
    ...
    serializer_class = MarketCotationSerializerWithNoCotations    
    permission_classes = []    
    ...
    def get_queryset(self):
        ...
        return (queryset, game_id)



class CotationByMarketView(CacheKeyDispatchMixin, ModelViewSet):
    queryset = Cotation.objects.none()
    serializer_class = CotationsByMarketSerializer
    permission_classes = []    
    cache_group = 'cotation_by_market_view'
    caching_time = 60 * 5
    ...
    def get_queryset(self):
        ...
        my_cotations_qs = Cotation.objects.filter(game=game_id, market__id=market_id).exclude(Q(market__name='1X2') | Q(pk__in=id_list_excluded_cotations))
        for market_removed in MarketRemotion.objects.filter(store=store):
            my_cotations_qs = my_cotations_qs.exclude(market__pk=market_removed.market_to_remove, name__regex=r'^.*?'+market_removed.under_above+'.*?'+market_removed.base_line+'.*?$')
        return my_cotations_qs
